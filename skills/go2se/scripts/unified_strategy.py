...
def main():
    ...
    # 配置通知
    if args.notify:
        go2se.config['notify']['enabled'] = True
        go2se.config['notify']['telegram'] = args.telegram_token
        go2se.config['notify']['telegram_chat_id'] = args.telegram_chat_id
        go2se.config['notify']['webhook'] = args.discord_webhook
