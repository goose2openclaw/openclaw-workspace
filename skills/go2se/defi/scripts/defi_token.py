#!/usr/bin/env python3
"""
GO2SE DeFi Token Creator - 智能代币生成器
...
contract {symbol} is ERC20, ERC20Burnable, Ownable {{
    ...
    def save_token(self, token_info):
        """保存代币配置"""
        os.makedirs('skills/go2se/defi/tokens', exist_ok=True)
        filename = f"skills/go2se/defi/tokens/{token_info['symbol']}_{token_info['chain']}.json"
    ...
    def list_tokens(self):
        ...
        import glob
        tokens = []
        for f in glob.glob('skills/go2se/defi/tokens/*.json'):
            with open(f) as fp:
