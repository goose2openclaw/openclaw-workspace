#!/usr/bin/env python3
"""
🪿 GO2SE vv6 Telegram 管理机器人
===================================
直接通过 Telegram 管理 GO2SE v6a 和 vv6 双系统
"""
...
# ─── GO2SE API 调用 ───────────────────────────────────────
def go2se_get(endpoint: str, version: str = "vv6") -> Dict:
    """调用 GO2SE API"""
    base = V6I_URL if version == "vv6" else V6A_URL
    url = f"{base}{endpoint}"
    try:
...
def main_keyboard():
    return {
        "inline_keyboard": [
            ...
            [{"text": "🖥️ v6a 后台", "callback_data": "v6a_status"}, {"text": "🤖 vv6 后台", "callback_data": "vv6_status"}],
...
def handle_command(text: str, chat_id: int, message_id: Optional[int] = None) -> str:
    ...
    # /start
    if text == "/start":
        return """🦢 *GO2SE vv6 Telegram 管理面板*

欢迎使用 GO2SE 北斗七鑫 Telegram 管理机器人！

*支持版本：*
• v6a — 多策略引擎 (端口 8000)
• vv6 — OpenAI Agents (端口 8001)
...
*vv6 后台 (端口 8001)*
状态: `{vv6.get('status', 'unknown')}`
版本: `{vv6.get('version', 'N/A')}`
...
*总资金:* 100,000 USDT"""
    ...
    # /performance
    if text == "/performance":
        data = go2se_get("/api/performance", "vv6")
        if "error" in data:
...
• 交易所: {deps.get('exchange', '❓')}"""
    ...
    # /vv6
    if text == "/vv6":
        data = go2se_get("/health", "vv6")
        agents = data.get("agents", [])
        perf = go2se_get("/api/performance", "vv6")
        return f"""🤖 *vv6 (OpenAI Agents) 详细状态*
...
*API Key:* {'✅ 已配置' if perf.get('openai_configured') else '❌ 未配置'}"""
    ...
    # /tools
    if text == "/tools":
        agents_resp = go2se_get("/api/agents", "vv6")
...
def handle_callback(callback_data: str, chat_id: int, message_id: int, callback_query_id: str) -> str:
    ...
    elif callback_data == "vv6_status":
        return handle_command("/vv6", chat_id)
...
def main():
    ...
    commands = [
        ...
        {"command": "vv6", "description": "vv6后台状态"},
