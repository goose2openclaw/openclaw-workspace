...
def collect_metrics() -> Dict:
    vv6_health = api_get(f"{VV6_URL}/health")
    v6a_health = api_get(f"{V6A_URL}/health")
    vv6_perf   = api_get(f"{VV6_URL}/api/performance")
    vv6_risk   = api_get(f"{VV6_URL}/api/risk/config")

    # POST 版分析
    vv6_switch = api_post(f"{VV6_URL}/api/switch/analyze",
...
def apply_adjustment(adj: Dict) -> bool:
    ...
    try:
        if t == "mode_switch":
            to_val = adj["to"]
            resp = api_post(f"{VV6_URL}/api/switch/mode?mode={to_val}", {})
