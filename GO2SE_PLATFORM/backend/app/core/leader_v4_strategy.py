...
class LeaderV4Strategy:
    ...
    # 支持的做市商
    MARKET_MAKERS = [
        "SpreadPro", "DepthMaster", "LiquidityKing", "BookMaker", 
        "MMElite", "ShortKing", "BearTrader", "BullRunner"
    ]
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.market_makers = self.MARKET_MAKERS
        
        if config:
            self.WEIGHTS.update(config.get("weights", {}))
        
        # 🔧 初始化5大核心能力
        self.enhancer = BeidouToolEnhancer(tool_name="leader", tool_type="market_making")
    ...
    async def mirofish_select(self, candidates: List[Dict] = None, strategy: str = "balanced", top_n: int = 5) -> List[Dict]:
        ...
        if candidates is None:
            candidates = [{"id": mm, "score": 0.5} for mm in self.market_makers]
    ...
    async def scan_with_all_capabilities(self, mode: str = "auto") -> Dict:
        ...
        # 3. MiroFish选品
        candidates = [
            {"id": r.get("target", mm), "score": r.get("score", 0.5)}
            for mm, r in zip(self.market_makers, global_results)
        ]
