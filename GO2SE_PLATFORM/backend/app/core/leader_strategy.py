...
class LeaderStrategy:
    ...
    def __init__(self,
                 symbol: str = "BTCUSDT",
                 interval: str = "1h",
                 whale_threshold: float = 50000.0,   # 大户门槛 USDT (从100K降至50K，兼顾大户和小鳄)
        ...
        self.consecutive_stops = 0  # V2: 连续止损计数，防止趋势跟踪失效时死循环
        self.entry_price = 0
    ...
    def detect_trend(self, klines: List[Dict]) -> Tuple[str, float]:
        ...
        # 计算动量 (V2: 附加确认)
        recent_returns = [(closes[i] - closes[i-1]) / closes[i-1] for i in range(-20, 0)]
        avg_return = sum(recent_returns) / len(recent_returns)
        momentum = 1 if avg_return > 0.0005 else -1 if avg_return < -0.0005 else 0

        # 趋势判断 (V2: 需要动量确认)
        if ema9 > ema21 > ema55 and current > ema9:
            base_conf = 0.6 + (ema9 - ema55) / ema55
            if momentum == -1:  # 动量背离，降权
                base_conf *= 0.7
            return "LONG", min(0.9, base_conf)
        elif ema9 < ema21 < ema55 and current < ema9:
            base_conf = 0.6 + (ema55 - ema9) / ema55
            if momentum == 1:  # 动量背离，降权
                base_conf *= 0.7
            return "SHORT", min(0.9, base_conf)
        else:
            return "NEUTRAL", 0.5
    ...
    def generate_signal(self) -> Optional[LeaderSignal]:
        ...
        # V2: 连续止损惩罚 - 如果近期多次被打止损，降低仓位
        if self.consecutive_stops >= 3:
            confidence *= 0.7
            logger.warning(f"⚠️ 连续{self.consecutive_stops}次止损，置信度降至{confidence:.2%}")
    ...
    def execute_signal(self, signal: LeaderSignal) -> Dict:
        ...
        self.consecutive_stops = 0  # V2: 重置连续止损计数
        ...
        if klines and self.position != 0:
            ...
            if should_exit:
                ...
                if reason == "STOP_LOSS":
                    ...
                    logger.warning(f"⚠️ 止损 #{self.consecutive_stops}，当前置信度惩罚: {(0.7**min(self.consecutive_stops-1,3)):.1%}")
                else:
                    self.consecutive_stops = 0  # 非止损平仓，重置计数
                self.position = 0
                self.entry_price = 0
                return result
